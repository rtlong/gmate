...
class AlignDialog(object):
    ...
    def __init__(self, parent):
        ...
        path = os.path.join(os.path.dirname(__file__), 'align3.glade')
        glade_xml = Gtk.Builder()
        glade_xml.add_from_file(path)
        self.dialog = glade_xml.get_object('dialog')
        self.entry = glade_xml.get_object('entry')
...
class AlignPluginWindow(GObject.Object, Gedit.WindowActivatable):
    __gtype_name__ = "AlignPluginWindowActivatable"
    ...
    def align(self, doc, bounds, separator):
        ...
        for i in reversed(range(len(lines))):
            ...
            text = doc.get_text(line_start, line_end, 0)
    ...
    def on_align_activate(self, *args):
        ...
        dialog.destroy()
        if response == Gtk.ResponseType.OK and separator:
